...
const Filters = ({ filters, spread }: Props) => {
    ...
    useEffect(() => {
        ...
        const currentFilter = params.get('filter') || 'none';
    ...
    return (
        <>
            {spread && <div className='mt-4 flex gap-3'>
                {filters.map(item => (
                    <Badge
                        className={`subtle-regular background-light700_dark300 text-light400_light500 cursor-pointer rounded-md border-none px-4 py-2 max-md:hidden ${item.value === filterValue && 'bg-primary-500 text-black'}`}
